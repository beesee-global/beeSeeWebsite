...
const Category = () => {
    ...
    const columns = [ 
        { id: 'name', label: 'Category Name', sortable: true, align: 'left' },
        { id: 'icon', label: 'Icon', sortable: true, align: 'left' },
    ];
    ...
    const filteredCategory = useMemo(() => {
        if (!debouncedSearch?.trim()) return categoryInfo 
            return categoryInfo.filter((u: any) => 
            u.name.toLowerCase().includes(debouncedSearch?.toLowerCase()) || 
            u.icon.toLowerCase().includes(debouncedSearch?.toLowerCase())
        )
    }, [categoryInfo, debouncedSearch]);
    ...
    const handleRowDoubleClick = (row:any) => {
        navigate(`/beesee/ecommerce/category/form/${row.pid}`)
    }
    ...
    const handleUpdate = () => {
        ...
        const selectedCategory = categoryInfo.find((j: any) => j.id === selectedRowId);
        if (selectedCategory) {
            navigate(`/beesee/ecommerce/category/form/${selectedCategory.pid}`)
        }
    }
