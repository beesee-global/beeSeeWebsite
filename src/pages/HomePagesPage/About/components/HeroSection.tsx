...
const UnifiedScrollingPage: React.FC = () => {
  ...
  const renderHeroContent = () => {
    const content = (
      <>
        {/* Hero Text */}
        <div className={`${isMobile ? "text-center" : "text-left"} space-y-1 md:space-y-2`}>
        ...
        {/* Body Text */}
        <p className={`bee-body max-w-xl text-[#C7B897] leading-relaxed mt-4 md:mt-8 px-2 md:px-0 ${isMobile ? "text-center" : "text-left"}`}>
        ...
        <div className={`flex flex-col sm:flex-row flex-wrap gap-4 md:gap-6 pt-4 md:pt-8 px-2 md:px-0 ${isMobile ? "items-center justify-center" : "items-start justify-start"}`}>
          <button
  ...
  const renderHeroCard = () => {
    const cardContent = (
      <div className="beesee-card-content section-two-card bg-[#000]/30 border border-[var(--beesee-gold)]/30 rounded-2xl backdrop-blur-lg shadow-[0_0_15px_rgba(253,204,0,0.08)] hover:scale-105 hover:border-[#FDCC00]/60 hover:shadow-[0_0_50px_rgba(253,204,0,0.15)] transition-transform duration-300 p-4 md:p-8">
        ...
        {showVideo && (
          <button
            ...
            className="bee-body-sm text-[var(--beesee-gold)] hover:text-white transition mt-4 w-full text-center block"
          >
            ✕ Close Video
          </button>
        )}
    ...
    if (isMobile) {
      return <div className="mt-6 px-2 md:px-0">{cardContent}</div>;
    }
  ...
  const renderMissionVisionCard = (icon: React.ReactNode, title: string, content: string, index: number) => {
    const card = (
      <div className="beesee-card-content hover:scale-105 hover:border-[#FDCC00]/40 hover:shadow-[0_0_15px_rgba(253,204,0,0.15)] transition-transform duration-300 rounded-2xl p-4 md:p-8 mx-2 md:mx-0">
