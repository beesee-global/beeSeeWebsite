...
const ProductInquiryModal: React.FC<ProductInquiryModalProps> = ({
  ...
  // inquiries_reply.subject is varchar(50); keep the public payload within
  // that limit while retaining the product name at the start.
  const subject = (selectedVariant
    ? `Product Inquiry: ${productName} (${productPid}) - ${selectedVariant}`
    : `Product Inquiry: ${productName} (${productPid})`).slice(0, 50);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // The API still expects a position property for compatibility with the
      // existing inquiry schema, but the public product form no longer asks
      // visitors to provide one.
      await createConsultation({ ...form, position: "", subject });
      setSubmitted(true);
    } catch {
    ...
    >
      <div
        ...
        style={{
          ...
          border: "1px solid rgba(253,204,0,.38)",
          borderLeft: "4px solid #FDCC00",
      ...
      >
        ...
        <div
          ...
          style={{ backgroundColor: "#FDCC00", opacity: 0.85 }}
        />
        ...
        {submitted ? (
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <CustomTextField name="company" placeholder="Enter company name (optional)" value={form.company} onChange={updateField} icon={<Building2 />} rows={1} maxLength={100} type="text" multiline={false} error={!!errors.company} helperText={errors.company} />
