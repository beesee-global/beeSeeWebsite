...
const HeroSection: React.FC = () => {
    ...
    const { data: productResponse = [] } = useQuery({
      ...
      select: (res) => {
          ...
          return mapped.sort((left: any, right: any) => {
              const leftSize = Number.parseInt(String(left.label).replace(/[^0-9].*$/, ''), 10);
              const rightSize = Number.parseInt(String(right.label).replace(/[^0-9].*$/, ''), 10);
              return (Number.isNaN(leftSize) ? Number.MAX_SAFE_INTEGER : leftSize)
                  - (Number.isNaN(rightSize) ? Number.MAX_SAFE_INTEGER : rightSize);
          });
      },
    });
    ...
    const selectedCategory = categoryResponse.find((item: any) => item.value === String(formData?.category_id));
    const selectedCategoryLabel = String(formData?.category_id) === '7'
        ? 'Educational Smart TV'
