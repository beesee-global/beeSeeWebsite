...
const renderSpecValue = (key: string, raw: any) => {
  ...
  const isWrappedValue = v.split(/\s+/).filter(Boolean).length >= 3;
  return (
    <div className={`spec-value-enhanced${isWrappedValue ? " spec-value-wrapped" : ""}`}>
      {formatSpecValue(key, v)}
    </div>
  );
};
...
const ProductCard: React.FC<{
  ...
  const hoverSpecs = React.useMemo(() => {
    const MAX_SPECS = 4;
    if (product.quickProductHighlightEnabled === false) return [];
  ...
  }, [product.quickHighlights, product.quickProductHighlightEnabled, product.specs, product.specIcons]);
    ...
    >
      ...
      >
        ...
        <div className="product-image-container">
          ...
          {isMobile && hoverSpecs.length > 0 && (
            <button
              type="button"
              className={`mobile-specs-toggle${mobileSpecsOpen ? " is-open" : ""}`}
              aria-label={`${mobileSpecsOpen ? "Hide" : "Show"} specifications for ${product.name}`}
              aria-expanded={mobileSpecsOpen}
              onClick={(event) => {
                event.preventDefault();
                event.stopPropagation();
                setMobileSpecsOpen((open) => !open);
              }}
            >
              {mobileSpecsOpen ? <X size={20} aria-hidden="true" /> : <Info size={20} aria-hidden="true" />}
            </button>
          )}

          {/* SPECS - Hover on desktop, explicit toggle on mobile */}
          <AnimatePresence>
            {(isHovered || (isMobile && mobileSpecsOpen)) && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
