...
const BootstrapDialog = styled(Dialog)(({ theme }) => ({
  '& .MuiDialog-paper': { width: '500px', maxWidth: '90%' },
        ...
        <form onSubmit={handleSubmit} id="reusable-form">
          {fields.map(field => {
            if (field.type === "select") {
              ...
              const options = field.name === "product" && field.options
                ? field.options.filter(p => p.categories_id?.toString() === selectedCategory)
                : field.options || [];
              ...
              return (
                <CustomSelectField
                    ...
                    onChange={(e: any) => handleSelectChange(field.name, e.target.value)} // <-- extract value
                    options={options}
