...
import {
    deleteJourney,
    fetchAllJourneys,
} from '../../../../services/ourJourneyServices'; 
...
const OurJourney = () => {
    ...
    // --- pass data on api ---
    const { 
        mutateAsync: deleteJourneyAsync, 
        isPending 
    } = useMutation({
    ...
    const journeyInfo = journeyResponse || []; // <-- this is now Journey[]
    ...
    const handleDeleteRow = async () => {
        try {
            await deleteJourneyAsync(deleteId);
    ...
    const handleRowEdit = (id: number | string) => {
        navigate(`/beesee/our-journey/form/${id}`);
    };
    ...
    const handleVerifyDelete = (id: number | string) => {
        setDeleteId(id);
        setOpenModal(true);
        setTitle('Delete Journey Item Confirmation');
        setMessage(`This action cannot be undone. Are you sure you want to delete this journey item?`);
    };
                ...
                <div className="mb-6">
                    <Breadcrumb
                        items={[
                            ...
                            { label: 'Our Journey', isActive: true, icon: <MapPin className="w-4 h-4" /> },
                        ]}
                    />
                </div>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-gray-600 dark:text-gray-400">
                                Manage your company's journey and milestones
                            </p>
                        </div>

                        <button
                            onClick={() => navigate('/beesee/our-journey/form')}
                    ...
                    {journeyInfo.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-600 dark:text-gray-400 mb-6">
                                Start adding journey milestones to tell your company's story
                            </p>
                            <button
                                onClick={() => navigate('/beesee/our-journey/form')}
                                className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-[#FCD000] to-[#FCD000]/90 hover:from-[#FCD000]/90 hover:to-[#FCD000] text-gray-900 rounded-lg font-semibold transition-all"
                            >
