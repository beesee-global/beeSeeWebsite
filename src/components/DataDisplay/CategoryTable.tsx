...
interface EnhancedTableProps {
  ...
  handleRowDelete: (id: number | string) => void;
}

const EnhancedTable: React.FC<EnhancedTableProps> = ({ rows, columns, handleRowEdit, handleRowDelete }) => { 
                  ...
                  >
                    {columns.map((column) => (
                      <td key={column.id} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                        <div className="flex items-center">
                          {column.id === 'image_url' && row[column.id] ? (
                            <img 
                              src={row[column.id]}
                              alt={row.name || 'image'}
                            ...
                            <span className='truncate max-w-xs'>{row[column.id]}</span>
                          )}
                        </div>
                      </td>
                    ))} 
